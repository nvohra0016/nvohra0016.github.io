...
In this post, we investigate two approaches to solving the heat equation on $\Omega \subset \mathbb{R}^3$
...
The temperature error over the entire simulation time period for approach (a) is $\lVert \overline{\theta^n} - \overline{\theta^n_{ref}} \rVert_\infty = 0.1590$ [$^\circ$ C] and for approach (b) $\lVert \overline{\theta^n} - \overline{\theta_{ref}^n} \rVert_\infty = 0.2323$ [$^\circ$ C], where $\theta^n_{ref}$ referes to the reference solution at time step $n$. The error shows a significant difference between the two approaches.
