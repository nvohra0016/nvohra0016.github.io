...
**Theorem** Let $F \in C^\infty (\mathbb{R})$ and let $x_{sol}$ be the unique solution $F(x_{sol}) = 0$. Suppose $J_x = F'(x) \neq 0, \; \forall x \in \mathbb{R}$. Then the sequence $\\{ x^{(m)} \\}$ generated by the algorithm \ref{eq:Newtons_method}-\ref{eq:Newtons_method2} converges to $x_{sol}$ for an appropriate initial guess $x^{(0)}$. 
